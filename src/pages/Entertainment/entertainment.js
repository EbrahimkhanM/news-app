...
function Entertainment() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      apiUrl = `https://newsapi.org/v2/everything?q=entertainment&apiKey=${newsAPIKey}&language=en&searchIn=title`;
      ...
      {loading ? (
        ...
        <div className="px-6 my-6 max-w-[1280px] mx-auto ">
          <div className=" flex flex-col md:flex-row gap-4 bg-[#F7F7F8]">
            <MainNewsCard news={news[1]} title="Entertainment" />
            <div>
              {news &&
                news.slice(6, 9).map((item, index) => (
                    <SideNewsCard title="Entertainment" key={index} news={item} />
                  ))}
            </div>
          </div>
          <div className="lg:mt-12 mt-28">
            <h3 className="mb-3 text-4xl font-semibold">Entertainment</h3>
            <div className="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
            {news.slice(1, showMore ? news.length : 7).map((item, index) => (
                <Card key={index} news={item} title="Entertainment" />
              ))}
            </div>
            {news.length > 6 && (
